...
@implementation GCSMiniControlCluster
...
- (GCSMiniControl *)controlForKey:(NSString *)key
{
	...
	GCSMiniControl *mc = mControlNames[key];

	if (mc == nil) {
		for (mc in self.controls) {
			if ([mc isKindOfClass:[GCSMiniControlCluster class]]) {
				mc = [(GCSMiniControlCluster *)mc controlForKey:key];
...
- (instancetype)initWithBounds:(NSRect)rect inCluster:(GCSMiniControlCluster *)clust
{
	...
	if (self != nil) {
		...
		mControlAlpha = 1.0;
		NSAssert(mHitPart == kDKMiniControlNoPart, @"Expected init to zero");
		NSAssert(mLinkPart == kDKMiniControlNoPart, @"Expected init to zero");
		NSAssert(mLinkModFlagsMask == 0, @"Expected init to zero");
		mVisible = YES;

		if (mControls == nil || mControlNames == nil) {
			return nil;
		}
	}
	return self;
