...
@implementation WTSGradientControl
	...
	for (DKColorStop *element in mUnsortedStops) {
		if (element != mDeletionCandidateRef) {
			...
			if (element == mSelectedStopRef || element == mDragStopRef) {
				if (mMouseDownInStop)
					state = kDKPressedState;
				else
					state = kDKSelectedState;
			} else
				state = kDKNormalState;
...
- (NSCursor *)makeCursorForDeletingStop:(DKColorStop *)stop
{
	...
	if (sCurs == nil || stop != sStop) {
		...
		[stopImg drawInRect:b fromRect:NSZeroRect operation:NSCompositingOperationCopy fraction:0.5 respectFlipped:YES hints:nil];
		[poofImage drawInRect:a fromRect:NSZeroRect operation:NSCompositingOperationSourceOver fraction:1.0 respectFlipped:YES hints:nil];
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
	...
	if (stop) {
		...
		[GCSWindowMenu popUpWindowMenu:popup atPoint:loc withEvent:theEvent forView:self];
		...
		item = (NSMenuItem *)[contextualMenu insertItemWithTitle:NSLocalizedString(@"Reverse Colors", @"")
...
- (void)awakeFromNib
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(externalStopChange:) name:kDKNotificationGradientDidRemoveColorStop object:nil];
}
