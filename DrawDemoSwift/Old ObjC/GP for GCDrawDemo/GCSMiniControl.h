...
@protocol GCSMiniControlDelegate;
...
//! but not vice versa.
@interface GCSMiniControl : NSObject {
	NSRect mBounds;												// area fully enclosing the control
	GCSMiniControlCluster *__weak mClusterRef;					// cluster we belong to, if any
	NSString *mIdent;											// control's identifier, if any
	id<GCSMiniControlDelegate> __unsafe_unretained mDelegateRef; // delegate, if any
...
@property (nonatomic, unsafe_unretained) id<GCSMiniControlDelegate> delegate;
...
@protocol GCSMiniControlDelegate <NSObject>
@optional

- (void)miniControl:(GCSMiniControl *)mc willChangeValue:(id)newValue;
